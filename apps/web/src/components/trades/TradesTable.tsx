...
export default function TradesTable({
  ...
  const ANNOTATION_FIELDS = new Set(['emotion', 'notes', 'screenshots', 'htfBias', 'session', 'marketCondition', 'analysisTimeframe', 'entryTimeframe', 'thesis', 'expectation', 'lesson', 'conviction']);

  const updateActiveTradeField = (key: keyof Trade | 'emotion' | 'notes' | 'screenshots' | 'htfBias' | 'session' | 'marketCondition' | 'analysisTimeframe' | 'entryTimeframe' | 'thesis' | 'expectation' | 'lesson' | 'conviction' | 'setup' | 'triggers' | 'confluences' | 'plan', value: any) => {
    if (!activeTradeId) return;
    setTrades(prev =>
      prev.map(t => {
