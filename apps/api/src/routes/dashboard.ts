...
function generateEdgeInsight(recentTrades: any[]): EdgeInsight {
  ...
  if (closedTrades.length < MIN_TRADES) {
    return {
      type: 'fallback',
      insight: `بعد از ${MIN_TRADES - closedTrades.length} معامله بیشتر، لبه معاملاتیت رو بهت نشون می‌دیم`,
      winRate: 0,
  ...
  return {
    type: 'fallback',
    insight: `بعد از ${MIN_TRADES} معامله بیشتر، لبه معاملاتیت رو بهت نشون می‌دیم`,
    winRate: 0,
    sampleSize: closedTrades.length,
  };
}
