...
import { TradeData, MulterFile } from '../types';

/** Raw CSV row from MT4/MT5 export (all string values) */
interface RawTrade {
  ticket?: string;
  time?: string;
  type?: string;
...
/** Parsed trade with required importSource for file-based imports */
interface ParsedTrade extends TradeData {
  importSource: 'MT4_CSV' | 'MT4_HTM' | 'MT5_CSV';
}

// Detect file type
function detectFormat(fileBuffer: Buffer): 'MT4_HTM' | 'MT4_CSV' | 'MT5_CSV' {
  const fileContent = fileBuffer.toString('utf-8', 0, 1024); // First 1KB
  if (fileContent.includes('<table')) return 'MT4_HTM';
...
// Parse MT4 HTML
async function parseMT4HTML(fileBuffer: Buffer): Promise<ParsedTrade[]> {
  const html = parse(fileBuffer.toString('utf-8'));
  const table = html.querySelector('table');
  if (!table) throw new Error('No table found in MT4 HTML');
  ...
  return Array.from(table.querySelectorAll('tr')).slice(1).map(row => {
    ...
    return {
      ...
      direction: cells[2].textContent?.trim() as 'BUY' | 'SELL',
...
// Main parse function
async function parseTrades(file: MulterFile): Promise<ParsedTrade[]> {
...
export { parseTrades, ParsedTrade };
